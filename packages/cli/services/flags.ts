...
export const FlagsQuerySchema = EnvironmentQuerySchema.extend({
  sortBy: z.string().default("key").describe("Field to sort flags by"),
  sortOrder: z
    .enum(["asc", "desc"])
    .default("asc")
...
export async function listFlags(appId: string, query: FlagsQuery) {
  return authRequest<FlagsResponse>(`/apps/${appId}/flags`, {
...
export async function createFlag(appId: string, flagData: FlagCreate) {
  return authRequest<CreateFlagResponse>(`/apps/${appId}/flags`, {
    method: "POST",
    headers: {
