...
import { baseUrlSuffix, flagUrl } from "../utils/urls.js";
...
export const createFlagAction = async (
  ...
  try {
    ...
    if (production) {
      console.log(
        indentLines(chalk.magenta(flagUrl(baseUrl, production, flag))),
      );
    }
  } catch (error) {
...
export const listFlagsAction = async () => {
  ...
  if (!appId) {
    handleError(new MissingAppIdError(), "Flags List");
  }
...
export function registerFlagsCommands(cli: Command) {
